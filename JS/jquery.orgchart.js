(function ($) {
    ...
    return this.each(function () {
      ...
      if (opts.levels > -1) {
        $this
          ...
          .andSelf()
          .filter(function () {
            return $chartSource.parents("ul").length + 1 > opts.levels;
          })
          .remove();
      }
  ...
  function buildNode($node, $appendTo, level, index, opts) {
    ...
    //aquí cunado le doy clic me da del id del nodo seleccionado
    $nodeDiv.click(function () {
      var $this = $(this);

      opts.nodeClicked($this.data("orgchart-node"));
      if (opts.interactive) {
        var nodeId = $node.eq(0).attr("id");
      ...
      //--seleccioon
              // Remover la clase de color de todos los nodos
              $("div.node").removeClass("selected");

              // Agregar la clase de color al nodo seleccionado
              $this.addClass("selected");
      
              // Restablecer el color original de los nodos hermanos
