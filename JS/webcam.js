...
import * as faceapi from '../API/dist/face-api.esm.js'; // use when in dev mode
// import * as faceapi from '@vladmandic/face-api'; // use when downloading face-api as npm

// configuration options
const modelPath = '../API/model/'; // path to model folder that will be loaded using http
...
function drawFaces(canvas, data, fps) {
  ...
  for (const person of data) {
    ...
    ctx.fillStyle = 'red';
   ctx.fillText(`Expresión (o.o): ${Math.round(100 * expression[0][1])}% ${expression[0][0]}`, person.detection.box.x, person.detection.box.y - 41);
...
//__________________________________________________
    // check REVISIÓN-Aqui obtengo la emoción
    //emociones por defecto:
    //FACE_EXPRESSION_LABELS = ['neutral', 'happy', 'sad', 'angry', 'fearful', 'disgusted', 'surprised'];
    // ETIQUETAS DE EXPRESIÓN CARA = ['neutral', 'feliz', 'triste', 'enojado', 'temeroso', 'disgustado', 'sorprendido'];
   //console.log(`emoción: ${expression[0][0]}`);
